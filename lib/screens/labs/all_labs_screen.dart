...
class _AllLabsScreenState extends State<AllLabsScreen> {
  ...
  List<Franchise> _filtered(List<Franchise> all) {
    List<Franchise> result =
        all.where((f) => f.status.toLowerCase() == 'active').toList();
    ...
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(18),
          onTap: () {
            // Navigate to franchise-aware booking flow using a generic test
            final dummyService = Service(
              id: franchise.franchiseId,
              name: 'Lab Tests',
              price: 0,
              durationMinutes: 30,
              description: 'Select from available tests at ${franchise.name}',
            );
            Navigator.push(
              context,
              MaterialPageRoute(
